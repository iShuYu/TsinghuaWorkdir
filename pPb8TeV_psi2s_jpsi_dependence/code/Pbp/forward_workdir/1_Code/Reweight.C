...
{
	//input
	int hi_lo = 74; //mean of PVNTRACKS, divide hi lo sample
	TH1::SetDefaultSumw2();
	...
	double PT, Y, N, w;
	TH1D *SJ_N = new TH1D("SJ_N","SJ_N",30,0,240);
	TH1D *SP_N = new TH1D("SP_N","SP_N",30,0,240);
	TH1D *DJ_N = new TH1D("DJ_N","DJ_N",30,0,240);
	TH1D *DP_N = new TH1D("DP_N","DP_N",30,0,240);
	TH1D *SJ_PT = new TH1D("SJ_PT","SJ_PT",20,0,14000);
	TH1D *SP_PT = new TH1D("SP_PT","SP_PT",20,0,14000);
	TH1D *SJ_Y = new TH1D("SJ_Y","SJ_Y",6,-5.5,-2.5);
	TH1D *SP_Y = new TH1D("SP_Y","SP_Y",6,-5.5,-2.5);
	...
	TH1D *SJ_hiY = new TH1D("SJ_hiY","SJ_hiY",6,-5.5,-2.5);
	TH1D *SP_hiY = new TH1D("SP_hiY","SP_hiY",6,-5.5,-2.5);
	TH1D *SJ_loY = new TH1D("SJ_loY","SJ_loY",6,-5.5,-2.5);
	TH1D *SP_loY = new TH1D("SP_loY","SP_loY",6,-5.5,-2.5);
	...
	TH1D *DJ_hiY = new TH1D("DJ_hiY","DJ_hiY",6,-5.5,-2.5);
	TH1D *DP_hiY = new TH1D("DP_hiY","DP_hiY",6,-5.5,-2.5);
	TH1D *DJ_loY = new TH1D("DJ_loY","DJ_loY",6,-5.5,-2.5);
	TH1D *DP_loY = new TH1D("DP_loY","DP_loY",6,-5.5,-2.5);
	//Fill Jpsi Splot
	for(int i=0; i<NSJ; i++)
	{
		SplotJ -> GetEntry(i);
		if(!(SJ.psi_OWNPV_Z<=120)) continue;
		w = SJ.sig_sw;
		N = SJ.psi_nForWardTracks;
		Y = SJ.psi_Y;
		PT = SJ.psi_PT;
		SJ_N -> Fill(N,w);
		SJ_PT -> Fill(PT,w);
	...
	{
		SplotP -> GetEntry(i);
		if(!(SP.psi_OWNPV_Z<=120)) continue;
		w = SP.sig_sw;
		N = SP.psi_nForWardTracks;
		Y = SP.psi_Y;
		PT = SP.psi_PT;
		SP_N -> Fill(N,w);
		SP_PT -> Fill(PT,w);
	...
	for(int i=0; i<NDJ; i++)
		...
		if(!(DJ.psi_OWNPV_Z<=120)) continue;
		...
		N = DJ.PVNTRACKS[0]-DJ.nBackTracks;
                Y = DJ.psi_Y;
                PT = DJ.psi_PT;
		DJ_N -> Fill(N);
		if(N>=hi_lo)
	...
	for(int i=0; i<NDP; i++)
        {
                ...
                if(!(DP.psi_OWNPV_Z<=120)) continue;
                ...
                N = DP.PVNTRACKS[0]-DP.nBackTracks;
                Y = DP.psi_Y;
                PT = DP.psi_PT;
		DP_N -> Fill(N);
                if(N>=hi_lo)
	...
	//Save
	TH1D *wJ_N = new TH1D("wJ_N","wJ_N",30,0,240);
	TH1D *wP_N = new TH1D("wP_N","wP_N",30,0,240);
	TH1D *wJ_hiPT = new TH1D("wJ_hiPT","wJ_hiPT",20,0,14000);
	TH1D *wP_hiPT = new TH1D("wP_hiPT","wP_hiPT",20,0,14000);
	TH1D *wJ_hiY = new TH1D("wJ_hiY","wJ_hiY",6,-5.5,-2.5);
        TH1D *wP_hiY = new TH1D("wP_hiY","wP_hiY",6,-5.5,-2.5);
	TH1D *wJ_loPT = new TH1D("wJ_loPT","wJ_loPT",20,0,14000);
        TH1D *wP_loPT = new TH1D("wP_loPT","wP_loPT",20,0,14000);
        TH1D *wJ_loY = new TH1D("wJ_loY","wJ_loY",6,-5.5,-2.5);
        TH1D *wP_loY = new TH1D("wP_loY","wP_loY",6,-5.5,-2.5);
