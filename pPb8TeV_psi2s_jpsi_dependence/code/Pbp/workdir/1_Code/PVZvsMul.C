...
void PVZvsMul()
{
	...
	F_inJ -> Add("../0_File/Data/RAW/Jpsi.root");
	F_inP -> Add("../0_File/Data/RAW/Psi2S.root");
	//plot
	...
	for(int i=0; i<10; i++)
	{
		TH1F *histJ=new TH1F(Form("histJ%d",i),"J/#psi, nPV=1",30,0,450);
		F_inJ->Draw(Form("PVNTRACKS[0]>>histJ%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
		histJ->Scale(1.0/histJ->Integral());
		histJ->SetLineColor(color[i]);
		histJ->SetMarkerColor(color[i]);
		histJ->SetStats(0);
		histJ->GetXaxis()->SetTitle("#font[12]{N}^{PV}_{tracks}");
                histJ->GetYaxis()->SetTitle("Normalized");
                histJ->GetXaxis()->SetTitleSize(0.06);
                histJ->GetYaxis()->SetTitleSize(0.06);
		if(i==0)
		{
			histJ->GetYaxis()->SetRangeUser(0,0.16);
		}
		histJ->Draw("same");
	...
	TCanvas *can2=new TCanvas("can2","can2",800,600);
        ...
        for(int i=0; i<10; i++)
        {
                TH1F *histP=new TH1F(Form("histP%d",i),"PVNTRACKS-PVZ (#psi(2S), nPV=1, rawdata)",30,0,450);
                F_inP->Draw(Form("PVNTRACKS[0]>>histP%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
                histP->Scale(1.0/histP->Integral());
                histP->SetLineColor(color[i]);
		histP->SetMarkerColor(color[i]);
                histP->SetStats(0);
                histP->GetXaxis()->SetTitle("#font[12]{N}^{PV}_{tracks}");
