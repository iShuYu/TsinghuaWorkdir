...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i<Ntree; i++)
	{
		...
		for(int j=0;j<N;j++)
		{
			th[j] -> Fill(t.PVNTRACKS[0],t.sig_sw);//change
		}
	}
	//calculate for
	...
	TGEp -> SetMarkerColor(kMagenta);
	TGEp -> SetLineColor(kMagenta);
        TGEb -> SetMarkerColor(kBlue);
	TGEb -> SetLineColor(kBlue);
        TGEp -> SetMarkerStyle(21);
        TGEb -> SetMarkerStyle(21);
	TGEp -> GetAttLine(0)->SetLineColor(kMagenta);
   	TGEp -> GetAttLine(1)->SetLineColor(kMagenta);
