...
int main(int argc, char* argv[])
{
	...
	TGEb -> SetMarkerColor(kCyan);
	TGEf -> SetLineColor(kGreen);
	TGEb -> SetLineColor(kCyan);
	TGEf -> SetMarkerStyle(21);
	TGEb -> SetMarkerStyle(21);
	TGEf -> GetAttLine(0)->SetLineColor(kGreen);
   	TGEf -> GetAttLine(1)->SetLineColor(kGreen);
   	TGEf -> GetAttFill(1)->SetFillStyle(0);
	TGEb -> GetAttLine(0)->SetLineColor(kCyan);
   	TGEb -> GetAttLine(1)->SetLineColor(kCyan);
   	TGEb -> GetAttFill(1)->SetFillStyle(0);
	TGEf -> GetXaxis() -> SetLimits(0,3);
	TGEf -> Draw("APS same; Z ; 5 s=1 ; same");
	TGEb -> Draw("PS same; Z ; 5 s=1 ; same");
	TGEf -> GetXaxis()->SetTitle("#font[12]{N}_{fwd}^{PV}/#LT#font[12]{N}_{fwd}^{PV}#GT_{NB}");
