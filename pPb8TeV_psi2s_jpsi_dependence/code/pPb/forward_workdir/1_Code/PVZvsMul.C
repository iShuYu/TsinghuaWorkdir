...
void PVZvsMul()
{
	...
	for(int i=0; i<10; i++)
	{
		...
		F_inJ->Draw(Form("psi_nForWardTracks>>histJ%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
	...
	TCanvas *can2=new TCanvas("can2","can2",800,600);
        ...
        for(int i=0; i<10; i++)
        {
                ...
                F_inP->Draw(Form("psi_nForWardTracks>>histP%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
