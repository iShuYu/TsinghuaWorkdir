...
int main(int argc, char* argv[])
{
	...
	TF1 *unit = new TF1("unit","10+x-x",0,3);
        unit -> SetLineStyle(2);
        unit -> SetLineColor(1);
        unit -> GetYaxis() -> SetRangeUser(0,0.03/rBR);
        unit -> GetXaxis()->SetTitle("#frac{nForwardTracks}{        <nForwardTracks>_{NoBias}}");
	...
	TGEb -> SetFillColorAlpha(kCyan,0.25);
	TGEf -> Draw("E2PZsame");
	TGEb -> Draw("E2PZsame");
