...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i<Ntree; i++)
	{
		...
		for(int j=0;j<N;j++)
		{
			th[j] -> Fill(t.psi_nForWardTracks,t.sig_sw);//change
		}
	}
	//calculate for
	...
	TGEp -> SetMarkerColor(kGreen);
	TGEp -> SetLineColor(kGreen);
        TGEb -> SetMarkerColor(kBlue);
	TGEb -> SetLineColor(kBlue);
        TGEp -> SetMarkerStyle(21);
        TGEb -> SetMarkerStyle(21);
	TGEp -> GetAttLine(0)->SetLineColor(kGreen);
   	TGEp -> GetAttLine(1)->SetLineColor(kGreen);
