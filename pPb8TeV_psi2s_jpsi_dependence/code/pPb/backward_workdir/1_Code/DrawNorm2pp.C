...
int main(int argc, char* argv[])
{
	//input nbin
	int N;
	double Nf_noBias = 27.37;
	double Nb_noBias = 22.47;
	double BRErr = 0.022;
	...
	ifstream Binb("/disk411/lhcb/kangye/Leadp/BWorkdir/3_Script/0_Bin/nbin");
	Binb >> N;
	for(int i=0;i<=N;i++)
	{
		Binb >> bin[i];
	...
	TGEf -> SetMarkerColor(kOrange+3);
	TGEb -> SetMarkerColor(kViolet);
	TGEf -> SetLineColor(kOrange+3);
	TGEb -> SetLineColor(kViolet);
	TGEf -> SetMarkerStyle(21);
	TGEb -> SetMarkerStyle(21);
	TGEf -> GetAttLine(0)->SetLineColor(kOrange+3);
   	TGEf -> GetAttLine(1)->SetLineColor(kOrange+3);
   	TGEf -> GetAttFill(1)->SetFillStyle(0);
	TGEb -> GetAttLine(0)->SetLineColor(kViolet);
   	TGEb -> GetAttLine(1)->SetLineColor(kViolet);
   	TGEb -> GetAttFill(1)->SetFillStyle(0);
	TGEf -> GetXaxis() -> SetLimits(0,3);
	TGEf -> Draw("APS same; Z ; 5 s=1 ; same");
	TGEb -> Draw("PS same; Z ; 5 s=1 ; same");
	TGEf -> GetXaxis()->SetTitle("#font[12]{N}_{bwd}^{PV}/#LT#font[12]{N}_{bwd}^{PV}#GT_{NB}");
