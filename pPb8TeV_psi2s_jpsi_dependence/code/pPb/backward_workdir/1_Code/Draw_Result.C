...
int main(int argc, char* argv[])
{
	...
	unit -> GetXaxis()->SetTitle("#font[12]{N}_{bwd}^{PV}/#LT#font[12]{N}_{fwd}^{PV}#GT_{NB}");
	...
	TGEp -> SetMarkerColor(kOrange+3);
	TGEp -> SetLineColor(kOrange+3);
        TGEb -> SetMarkerColor(kBlue);
	TGEb -> SetLineColor(kBlue);
        TGEp -> SetMarkerStyle(21);
        TGEb -> SetMarkerStyle(21);
	TGEp -> GetAttLine(0)->SetLineColor(kOrange+3);
   	TGEp -> GetAttLine(1)->SetLineColor(kOrange+3);
