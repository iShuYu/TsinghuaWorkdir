...
int main(int argc, char *argv[])
{
	...
	RooRealVar meanP("mean","mean mass of Psi2S from PDG",mp,mp-10,mp+10);
	RooRealVar sigmaP("sigmaP","sigma of CB for Psi2S",sp,sp-5,sp+5);
	RooFormulaVar alphaP("alphaP","alphaP","a*sigmaP*sigmaP+b*sigmaP+c",RooArgSet(a,b,c,sigmaP));
	...
	txtresult << sigmaP.getVal() << endl;
	txtresult << alphaP.getVal() << endl;
	txtresult << p0P.getVal() << endl;
	txtresult.close();

	double sig_sw, bkg_sw , psi_M, psi_M_check;
	TFile* splot_file = new TFile(argv[6], "recreate");
        RooStats::SPlot* splot = new RooStats::SPlot("splot", "splot", data_setJ, &massPDF_J, RooArgList(NsigJ, NbkgJ));
        TTree* result1 = input_treeJ->CloneTree(0);
        result1->Branch("sig_sw", &sig_sw, "sig_sw/D");
        result1->Branch("bkg_sw", &bkg_sw, "bkg_sw/D");
        input_treeJ->SetBranchAddress("psi_M",&psi_M);
        for (int i = 0; i < NJ; i++) {
                input_treeJ->GetEntry(i);
                psi_M_check = ((RooRealVar*)(data_setJ.get(i)->find("psi_M")))->getVal();
                if(psi_M!=psi_M_check) continue;
                sig_sw = ((RooRealVar*)(data_setJ.get(i)->find("NsigJ_sw")))->getVal();
                bkg_sw = ((RooRealVar*)(data_setJ.get(i)->find("NbkgJ_sw")))->getVal();
                result1->Fill();
	}
        result1->Write();
        splot_file->Close();

	TFile* splot_fileP = new TFile(argv[7], "recreate");
        RooStats::SPlot* splotP = new RooStats::SPlot("splotP", "splotP", data_setP, &massPDF_P, RooArgList(NsigP, NbkgP));
        TTree* result2 = input_treeP->CloneTree(0);
        result2->Branch("sig_sw", &sig_sw, "sig_sw/D");
        result2->Branch("bkg_sw", &bkg_sw, "bkg_sw/D");
        input_treeP->SetBranchAddress("psi_M",&psi_M);
        for (int i = 0; i < NP; i++) {
                input_treeP->GetEntry(i);
                psi_M_check = ((RooRealVar*)(data_setP.get(i)->find("psi_M")))->getVal();
                if(psi_M!=psi_M_check) continue;
                sig_sw = ((RooRealVar*)(data_setP.get(i)->find("NsigP_sw")))->getVal();
                bkg_sw = ((RooRealVar*)(data_setP.get(i)->find("NbkgP_sw")))->getVal();
                result2->Fill();
	}
        result2->Write();
        splot_fileP->Close();
