...
int main(int argc, char* argv[])
{
	...
	//TString A(argv[5]);
	//TString B(argv[9]);
	ifstream Ptot(argv[5]);
	//ifstream Btot(argv[10]);
	double y[1][3], pt[1][5], tot[1];
	for(int i=1;i<=3;i++)
	{
		y[0][i-1]=Py->GetBinContent(i)*100;
		//y[1][i-1]=By->GetBinContent(i); 
	}
	for(int i=1;i<=5;i++)
        {
                pt[0][i-1]=Ppt->GetBinContent(i)*100;
                //pt[1][i-1]=Bpt->GetBinContent(i);
        }
	//if(A==B)
	//{
		//Ptot >> tot[0];
	...
	out << argv[8] << Form(" for PVNTRACKS from %d to %d.}",PVN[Z-1],PVN[Z]) << endl;
	out << "\\begin{center}" << endl;
	out << "    \\begin{tabular}{ c | c }" << endl;
	out << "        \\hline" << endl;
        out << "        Region & prompt and b decay (\\%)\\\\" << endl;
        out << "        \\hline" << endl;
	out << "        2.0$<$y$<$2.8" << "&" << y[0][0] << "\\\\" << endl;
	out << "        2.8$<$y$<$3.5" << "&" << y[0][1] << "\\\\" << endl;
	out << "        3.5$<$y$<$4.5" << "&" << y[0][2] << "\\\\" << endl;
        out << "        \\hline" << endl;
	out << "        0\\gevc $<$\\pt$<$2\\gevc" << "&" << pt[0][0] << "\\\\" << endl;
	out << "        2\\gevc $<$\\pt$<$4\\gevc" << "&" << pt[0][1] << "\\\\" << endl;
	out << "        4\\gevc $<$\\pt$<$6\\gevc" << "&" << pt[0][2] << "\\\\" << endl;
	out << "        6\\gevc $<$\\pt$<$8\\gevc" << "&" << pt[0][3] << "\\\\" << endl;
	out << "        8\\gevc $<$\\pt$<$20\\gevc" << "&" << pt[0][4] << "\\\\" << endl;
        out << "        \\hline" << endl;
        out << "        all \\pt-y region&" << 100*tot[0] << "\\\\" << endl;
        out << "        \\hline" << endl;
