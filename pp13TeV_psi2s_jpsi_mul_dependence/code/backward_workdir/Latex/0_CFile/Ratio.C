...
int main(int argc, char* argv[])
{
	...
	//outut
	ofstream out(Form("../2_Table/Ratio/n%db.tex",N));
	out.setf(ios::fixed, ios::floatfield);
        out.precision(2);
	out << "\\begin{table}[H]" << endl;
	out << Form("\\caption{Ratios (\\%) of double differential production cross-section for \\psitwos to \\jpsi in bins of (\\pt,$y$) for %d $\\leq$ $N_{\\rm bwd}^{\\rm PV}$ $<$ %d. The first uncertainties are statistical, the second are systematic. }",PVN[N-1],PVN[N]) << endl;
	out << "\\centering" << endl;
	...
	out << "\\hline" << endl;
        ...
        out << "}" << endl;
        out << Form("\\label{RatioBACK%d}",N) << endl;
        out << "\\end{table}" << endl;
        out.close();	
	return 0;
}
