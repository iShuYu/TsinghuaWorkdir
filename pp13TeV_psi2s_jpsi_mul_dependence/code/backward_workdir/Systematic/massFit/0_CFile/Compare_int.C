...
int main(int argc, char *argv[])
{
	...
	//Tpt->SetBinContent(pt,(Sum_MsigP/Sum_MsigJ-Sum_sigP/Sum_sigJ)/(Sum_sigP/Sum_sigJ));
	//Tpt->SetBinError(pt,0);
	//TFile *out = new TFile(Form("../3_Result/massFitErr/n%d_sumPt_Err.root",N),"recreate");
	//Tpt->Write();
	//out->Close();
	ofstream out(Form("../3_Result/massFitErr/n%d_int_Err.txt",N));
	out << (Sum_MsigP/Sum_MsigJ-Sum_sigP/Sum_sigJ)/(Sum_sigP/Sum_sigJ) << endl;
	return 0;
}
