...
int main(int argc, char* argv[])
{
	...
	TH1D *Dp_N = new TH1D("Dp_N","Dp_N",60,0,120);
	TH1D *Db_N = new TH1D("Db_N","Db_N",60,0,120);;
	...
	Sp_N -> GetYaxis() -> SetRangeUser(0,0.1);
	Sp_N -> GetXaxis() -> SetTitle("#font[12]{N}^{PV}_{bwd}");
	...
	TCanvas *canb = new TCanvas("canb","canb",800,600);
        ...
        Sb_N -> GetYaxis() -> SetRangeUser(0,0.1);
        Sb_N -> GetXaxis() -> SetTitle("#font[12]{N}^{PV}_{bwd}");
