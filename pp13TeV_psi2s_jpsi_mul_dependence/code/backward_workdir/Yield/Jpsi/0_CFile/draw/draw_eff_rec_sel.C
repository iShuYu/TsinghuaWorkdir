#include <TAxis.h>
#include <TCanvas.h>
#include <TLegend.h>
#include <TROOT.h>
#include <TStyle.h>
#include <TFile.h>
#include <TH1.h>
#include <TH2.h>
#include "../header/lhcbStyle.h"
#include <fstream>
using namespace std;
int main(int argc, char* argv[])
{
	//gStyle->SetPaintTextFormat(".2f");
	int num_n;
	sscanf(argv[7],"%d",&num_n);
	vector<double> n, y, pt;
	double nbin, ybin, ptbin;
        int Nnbin, Nybin, Nptbin;
        ifstream nconfig(argv[4]);
        ifstream yconfig(argv[5]);
        ifstream ptconfig(argv[6]);
        nconfig >> Nnbin;
        yconfig >> Nybin;
        ptconfig >> Nptbin;
        for(int i=0; i<=Nnbin; i++)
        {
                nconfig >> nbin;
                n.push_back(nbin);
        }
        for(int i=0; i<=Nybin; i++)
        {
                yconfig >> ybin;
                y.push_back(ybin);
        }
        for(int i=0; i<=Nptbin; i++)
        {
                ptconfig >> ptbin;
                pt.push_back(ptbin);
        }
	double color[Nybin], marker[Nybin];
	for(int i=0; i<Nybin; i++)
        {
                color[i] = i+1;
                marker[i] = 20+i*2;
        }
	gROOT->SetBatch(true);
	setLHCbStyle();
	gStyle->SetPaintTextFormat(".3f");
	TFile input(argv[1]);
	TH2D *h_eff = (TH2D*)input.Get("h_eff");
	TCanvas* c0 = new TCanvas("c0", "c0", 0, 0, 800, 600);
	c0->SetRightMargin(0.16);
	h_eff->GetXaxis()->SetTitle("#font[12]{p}_{T}[MeV/#font[12]{c}]");
        h_eff->GetYaxis()->SetTitle("#font[12]{y}");
        h_eff->SetMarkerSize(1);
	h_eff->SetTitle(Form("%d <= n < %d",n[num_n-1],n[num_n]));
        h_eff->Draw("colz text e");
	c0->SaveAs(argv[2]);
        c0->Close();
	TCanvas* c1 = new TCanvas("c1","c1", 800, 600);
        TLegend* leg = new TLegend(0.65, 0.18, 0.85, 0.4);
        leg->SetTextFont(132);
        leg->SetTextSize(0.04);
	TH1D *rec_sel[Nybin];
	for(int i=0;i<Nybin;i++)
        {
		rec_sel[i] = (TH1D*)input.Get(Form("rec_sel%d",i));
                rec_sel[i]->SetLineColor(color[i]);
                rec_sel[i]->SetMarkerColor(color[i]);
                rec_sel[i]->SetMarkerStyle(marker[i]);
                if(i==0) rec_sel[i]->Draw();
                else rec_sel[i]->Draw("same");
        }
	for(int i=0;i<Nybin;i++)
	{
        	leg->AddEntry(Form("rec_sel%d",i), Form("%1.1f < #font[12]{y} < %1.1f", y[i], y[i+1]));
        }
	leg->Draw();
	c1->SaveAs(argv[3]);
        c1->Close();
	gROOT->ProcessLine(".q");
	return 0;
}
