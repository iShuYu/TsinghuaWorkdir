...
int main(int argc, char* argv[])
{
	...
	h_eff->GetXaxis()->SetTitle("#font[12]{p}_{T}[MeV/#font[12]{c}]");
        h_eff->GetYaxis()->SetTitle("#font[12]{y}");
        h_eff->SetMarkerSize(1);
	h_eff->SetTitle(Form("%d <= n < %d",n[num_n-1],n[num_n]));
        h_eff->Draw("colz text e");
	c0->SaveAs(argv[2]);
        c0->Close();
	TCanvas* c1 = new TCanvas("c1","c1", 800, 600);
        TLegend* leg = new TLegend(0.65, 0.13, 0.85, 0.35);
        leg->SetTextFont(132);
        leg->SetTextSize(0.04);
	TH1D *tot[Nybin];
	for(int i=0;i<Nybin;i++)
        {
		tot[i] = (TH1D*)input.Get(Form("tot%d",i));
                tot[i]->SetLineColor(color[i]);
                tot[i]->SetMarkerColor(color[i]);
                tot[i]->SetMarkerStyle(marker[i]);
                if(i==0) tot[i]->Draw();
                else tot[i]->Draw("same");
        }
	for(int i=0;i<Nybin;i++)
	{
        	leg->AddEntry(Form("tot%d",i), Form("%1.1f < #font[12]{y} < %1.1f", y[i], y[i+1]));
        }
	leg->Draw();
	c1->SaveAs(argv[3]);
        c1->Close();
