...
TTree* Tree::Select()
{
        ...
        for (long jentry=0; jentry<nentries; jentry++)
        {
                ...
                tz = (psi_ENDVERTEX_Z - psi_OWNPV_Z) / psi_PZ * 10.0 * 3096.9 / 2.9979;
                tzError = sqrt(psi_ENDVERTEX_ZERR * psi_ENDVERTEX_ZERR + psi_OWNPV_ZERR * psi_OWNPV_ZERR) / psi_PZ * 10.0 * 3096.9 / 2.9979;
...
Int_t Tree::Cut()
{
	if(!(
			...
			(psi_M <= 3096.9 - 50 || psi_M >= 3096.9 + 50) &&
			-10. < tz && tz < 10. &&
			tzError < 0.3 &&
			psi_L0DiMuonDecision_TOS == 1 &&
			psi_Hlt1DiMuonHighMassDecision_TOS == 1 &&
			//psi_Hlt2DiMuonJPsiTurboDecision_TOS == 1 &&
