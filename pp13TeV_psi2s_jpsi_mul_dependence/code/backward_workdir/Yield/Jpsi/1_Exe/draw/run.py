...
import time
#inpit
with open('../../../rawdata/3_Bins/nbin','r') as fN:
	n = fN.readlines()
	Nn = int(n[0])
with open('../../../rawdata/3_Bins/ybin','r') as fy:
        y = fy.readlines()
        Ny = int(y[0])
with open('../../../rawdata/3_Bins/ptbin','r') as fpt:
        pt = fpt.readlines()
        Npt = int(pt[0])
#parameters
core = int(sys.argv[1])
...
while flag:
	...
	if core > 0 and rest > 0:
		...
		ps = subprocess.Popen("%s %d %d %d"%(sys.argv[2],nn,yy,pp), shell = True )
