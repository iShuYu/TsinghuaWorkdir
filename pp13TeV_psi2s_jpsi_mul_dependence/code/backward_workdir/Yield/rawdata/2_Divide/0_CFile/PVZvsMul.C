void PVZvsMul()
{
	...
	for(int i=0; i<10; i++)
	{
		TH1F *hist=new TH1F(Form("hist%d",i),"J/#psi, nPV=1",30,0,90);
		F_inJ->Draw(Form("psi_nBackTracks>>hist%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
		hist->Scale(1.0/hist->Integral());
		hist->SetLineColor(color[i]);
		hist->SetStats(0);
		hist->GetXaxis()->SetTitle("nBackTracks");
	...
	TCanvas *can2=new TCanvas("can2","can2",800,600);
        ...
        for(int i=0; i<10; i++)
        {
                ...
                F_inP->Draw(Form("psi_nBackTracks>>hist%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
                hist->Scale(1.0/hist->Integral());
                hist->SetLineColor(color[i]);
                hist->SetStats(0);
		hist->GetXaxis()->SetTitle("nBackTracks");
