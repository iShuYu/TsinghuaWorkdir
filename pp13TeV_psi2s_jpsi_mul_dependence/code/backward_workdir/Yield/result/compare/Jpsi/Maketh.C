void maketh()
{
	...
	ifstream R("./test.C");
	TH2D *compare = new TH2D("compare","",12,ptbin,5,ybin);
...
*/
	for(int i=1; i<=12; i++)
	{
		for(int j=1; j<=5; j++)
		{
			compare -> SetBinContent(i,j,X[i-1][j-1]);
			compare -> SetBinError(i,j,Xerr[i-1][j-1]);
	...
	for(int i=0;i<5;i++)
	{
		...
		p_bins[i]->SetTitle("J/#psi (Prompt)");
		for(int j=0;j<14;j++)
		{
	...
	TCanvas *Rcanp = new TCanvas("Rcanp","Rcanp",1600,1200);
        ...
        for(int i=0;i<5;i++)
        {
                ...
                rp_bins[i]->GetYaxis()->SetRangeUser(0.,2.);
                ...
                rp_bins[i]->SetTitle("J/#psi (Prompt)");
                for(int j=0;j<14;j++)
                {
	...
	TCanvas *canb = new TCanvas("canb","canb",1600,1200);
		...
		b_bins[i]->SetTitle("J/#psi (From b)");
                for(int j=0;j<14;j++)
                {
	...
	TCanvas *Rcanb = new TCanvas("Rcanb","Rcanb",1600,1200);
        ...
        for(int i=0;i<5;i++)
        {
                ...
                rb_bins[i]->GetYaxis()->SetRangeUser(0.,2.);
                ...
                rb_bins[i]->SetTitle("J/#psi (From b)");
                for(int j=0;j<14;j++)
                {
