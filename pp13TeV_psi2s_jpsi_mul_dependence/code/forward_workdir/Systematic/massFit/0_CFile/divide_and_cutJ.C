...
int main(int argc, char *argv[])
{
	...
	C -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/MC/MagUp/Jpsi/MCJpsi16MagUp_b.root");
	C -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/MC/MagDown/Jpsi/MCJpsi16MagDown_b.root");
	MCJpsi t(C);
	//output
	TFile *C_out = new TFile(Form("../2_Rootfile/Jpsi/n%dy%dpt%d.root",num_n,num_y,num_pt),"recreate");
	...
	for(long jentry = 0; jentry <= Num_Entries; jentry++)
	{
		...
		if(!(
			t.nPVs == 1 &&
			t.PVNTRACKS[0]-t.nBackTracks >=n[num_n-1] &&
			t.PVNTRACKS[0]-t.nBackTracks < n[num_n] &&
			psi_Y  >= y[num_y-1]      &&
                        psi_Y < y[num_y]          &&
