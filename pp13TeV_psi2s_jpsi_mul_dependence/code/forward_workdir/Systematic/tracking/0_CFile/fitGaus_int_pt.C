...
int main(int argc, char *argv[])
{
	...
	int num_bin = 15;
        ...
        for(int y=0;y<1;y++)
        {
                for(int pt=0;pt<npt;pt++)
                {
                        ...
                        for(int i=0;i<200;i++)
                        {
                                mean[pt][y]=mean[pt][y]+Content[pt][i];
                        }
                        mean[pt][y]=mean[pt][y]/200;
                        std[pt][y]=0;
                        for(int i=0;i<200;i++)
                        {
                                std[pt][y]=std[pt][y]+(Content[pt][i]-mean[pt][y])*(Content[pt][i]-mean[pt][y]);
                        }
	...
	TH1D *GausErrTable = (TH1D*)GausErrTable2->ProjectionX();
	TCanvas *can[npt][1];
        for(int y=0;y<1;y++)
        {
                for(int pt=0;pt<npt;pt++)
                {
                        ...
                        if(Type==1) can[pt][y]->SaveAs(Form("../4_Plot/prompt_int_pt%d.pdf",pt+1));
			if(Type==2) can[pt][y]->SaveAs(Form("../4_Plot/fromb_int_pt%d.pdf",pt+1));
			if(Type==3) can[pt][y]->SaveAs(Form("../4_Plot/total_int_pt%d.pdf",pt+1));
                        can[pt][y]->Close();
                }
        }
        GausErrTable->Write();
