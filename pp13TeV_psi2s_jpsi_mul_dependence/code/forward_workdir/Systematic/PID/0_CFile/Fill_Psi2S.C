...
int main(int argc, char *argv[])
{
	...
	TFile PIDCalibU("../2_Rootfile/PID16MagUp.root");
        TH3F *tableU = (TH3F*)PIDCalibU.Get(Form("t%d",index));
        TFile PIDCalibD("../2_Rootfile/PID16MagDown.root");
        TH3F *tableD = (TH3F*)PIDCalibD.Get(Form("t%d",index));
	...
	for(int i=0; i<NU; i++)
	{
		...
		mum_P = tU.mum_P;
		mup_P = tU.mup_P;
		nN = tU.PVNTRACKS[0]-tU.nBackTracks;
		nSPD = tU.nSPDHits;
	...
	for(int i=0; i<ND; i++)
        {
                ...
                mum_P = tD.mum_P;
                mup_P = tD.mup_P;
                nN = tD.PVNTRACKS[0]-tD.nBackTracks;
		nSPD = tD.nSPDHits;
