...
int main(int argc, char *argv[])
{
	...
	F_DecayTree -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_i.root");
	F_MCDecayTree -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_i.root");
	F_DecayTree -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_l.root");
        F_MCDecayTree -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_l.root");
	//F_DecayTree -> Add("../../1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_k.root");
	//F_MCDecayTree -> Add("../../1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_k.root");
	//F_DecayTree -> Add("../../1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_i.root");
	//F_DecayTree -> Add("../../1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_i.root");
	MCPsi2S tree(F_DecayTree);
	mctPsi2S Tree(F_MCDecayTree);

	//output 1
	TFile *F_DecayTree_out = new TFile(Form("../2_AfterDivide/MC/Psi2S/n%d_MagUp.root",num_n),"recreate");
	...
	for(long jentry = 0; jentry <= Num_Entries; jentry++)
	{
		...
		if(!(
			tree.nPVs == 1 &&
			tree.PVNTRACKS[0]-tree.nBackTracks >=n[num_n-1] &&
			tree.PVNTRACKS[0]-tree.nBackTracks < n[num_n]
		)) continue;
		DecayTree_out->Fill();
	}
	DecayTree_out->Write();
	...
	//output 1 fromb
	TFile *F_DecayTree_out_fromb = new TFile(Form("../2_AfterDivide/MC/Psi2S/n%d_MagUp_fromb.root",num_n),"recreate");
	...
	//output 1 prompt
	TFile *F_DecayTree_out_prompt = new TFile(Form("../2_AfterDivide/MC/Psi2S/n%d_MagUp_prompt.root",num_n),"recreate");
	...
	//output 2
        TFile *F_MCDecayTree_out = new TFile(Form("../2_AfterDivide/MC/Psi2S/mctn%d_MagUp.root",num_n),"recreate");
	...
	//output 2 fromb
	TFile *F_MCDecayTree_out_fromb = new TFile(Form("../2_AfterDivide/MC/Psi2S/mctn%d_MagUp_fromb.root",num_n),"recreate");
	...
	//output 2 prompt
	TFile *F_MCDecayTree_out_prompt = new TFile(Form("../2_AfterDivide/MC/Psi2S/mctn%d_MagUp_prompt.root",num_n),"recreate");
