...
int main(int argc, char* argv[])
{
	...
	TChain *F_in = new TChain("DecayTree");
		F_in -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/data/MagUp/JpsiUp.root");
                F_in -> Add("~/workdir_local/Yield/rawdata/1_Rootfile/data/MagDown/JpsiDown.root");
	DecayTree tree(F_in);
	//select and output
	TFile *F_out = new TFile(Form("../2_AfterDivide/data/Jpsi/n%dy%dpt%d.root",num_n,num_y, num_pt),"recreate");
	...
	for(long jentry = 0; jentry <= Num_Entries; jentry++)
	{
		...
		//if(tree.L0DUTCK!=5647||tree.L0DUTCK!=5650) continue;
		double psi_Y = 0.5 * log((tree.psi_PE + tree.psi_PZ)/(tree.psi_PE - tree.psi_PZ));
		if(!(
					tree.psi_nForWardTracks >=n[num_n-1] &&
					tree.psi_nForWardTracks < n[num_n]    &&
