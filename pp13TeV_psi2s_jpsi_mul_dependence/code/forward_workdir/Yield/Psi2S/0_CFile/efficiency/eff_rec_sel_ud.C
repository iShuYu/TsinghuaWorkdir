...
int main(int argc, char *argv[])
{
	...
	for(int i=0; i<NBU; i++)
	{
		...
		nweight = reweightN->GetBinContent(reweightN->FindBin(BU.PVNTRACKS[0]-BU.nBackTracks));
                if(BU.PVNTRACKS[0]-BU.nBackTracks<0) nweight =1;
	...
	for(int i=0; i<NAU; i++)
	{
		...
		nweight = reweightN->GetBinContent(reweightN->FindBin(AU.PVNTRACKS[0]-AU.nBackTracks));
                if(AU.PVNTRACKS[0]-AU.nBackTracks<0) nweight =1;
	...
	for(int i=0; i<NBD; i++)
        {
                ...
                nweight = reweightN->GetBinContent(reweightN->FindBin(BD.PVNTRACKS[0]-BD.nBackTracks));
                if(BD.PVNTRACKS[0]-BD.nBackTracks<0) nweight =1;
		ptyweight = reweightPTY->GetBinContent(reweightPTY->FindBin(mctPT,mctY));
                //yweight = reweightY->GetBinContent(reweightY->FindBin(mctY));
                h_B -> Fill(mctPT, mctY, nweight*ptyweight);
        }
		...
		if(mum_Tweight==0) mum_Tweight = 1.;
                if(mup_Tweight==0) mup_Tweight = 1.;
		nweight = reweightN->GetBinContent(reweightN->FindBin(AD.PVNTRACKS[0]-AD.nBackTracks));
                if(AD.PVNTRACKS[0]-AD.nBackTracks<0) nweight =1;
		ptyweight = reweightPTY->GetBinContent(reweightPTY->FindBin(DTPT,DTY));
                //yweight = reweightY->GetBinContent(reweightY->FindBin(DTY));
                h_A -> Fill(DTPT, DTY, mum_Tweight*mup_Tweight*nweight*ptyweight);
        }
	...
	TH1D* rec_sel[Nybin];
        for (int i=0; i<Nybin; i++)
        {
                ...
                rec_sel[i]->GetYaxis()->SetTitle(argv[11]);
