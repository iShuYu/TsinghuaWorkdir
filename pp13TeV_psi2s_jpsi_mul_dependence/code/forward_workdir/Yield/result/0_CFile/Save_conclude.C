...
{
        //We only calculate content here, all the Errors are combined in Systematic, actually I include the Stats there too. Even if I call the directory "Systematic"
	//modified: only calculate the sysErr
	setLHCbStyle();
	TFile input(argv[1]);
	...
	//1. modification over all pty bin
	...
	for(int i=1; i<=Nn; i++)
		...
		ifstream E(Form("../../../Systematic/AllErr/3_Result/n%d_int_OnlySysErr.txt",i));
			...
			//allp -> SetBinError(i,sqrt(SumEPp/SumCPp/SumCPp+SumEJp/SumCJp/SumCJp)*SumCPp/SumCJp);
			allp -> SetBinError(i,SumCPp/SumCJp*Errp*PJpsi/PPsi2S);
			allb -> SetBinContent(i,SumCPb/SumCJb*BJpsi/BPsi2S);
		       //allb -> SetBinError(i,sqrt(SumEPb/SumCPb/SumCPb+SumEJb/SumCJb/SumCJb)*SumCPb/SumCJb);
		...
		allp -> Write();
		allb -> Write();
		//2. modification of each pt range
		TH1D *pPt[Npt];
		TH1D *bPt[Npt];
		for(int k=1; k<=Npt; k++)
		{
		...
		{   
			...
			{   
				...
				//cout << "Errb" << Errb*SumCPb/SumCJb << endl;
				E->Close();
			}   
		}
		out->cd();
