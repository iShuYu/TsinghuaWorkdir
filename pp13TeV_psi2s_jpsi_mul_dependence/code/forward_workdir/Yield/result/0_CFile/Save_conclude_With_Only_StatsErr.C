...
int main(int argc, char* argv[])
{
	...
	for(int i=1; i<=Nn; i++)
	{
		ifstream E(Form("../../../Systematic/AllErr/3_Result/n%d_int_OnlyStatsErr.txt",i));
	...
	for(int k=1; k<=Npt; k++)
	{
		...
		for(int i=1; i<=Nn; i++)
		{
			...
			double SumCPp = 0;
			double SumCPb = 0;
			TFile *E = TFile::Open(Form("../../../Systematic/AllErr/3_Result/n%d_int_pt_OnlyStatsErr.root",i));
			double Errp, Errb;
	...
	{   
		...
		{   
			...
			TH2D *Pp = (TH2D*)input.Get(Form("Pp%d",i));
			TH2D *Pb = (TH2D*)input.Get(Form("Pb%d",i));
			TFile *E = TFile::Open(Form("../../../Systematic/AllErr/3_Result/n%d_int_y_OnlyStatsErr.root",i));
			double Errp, Errb;
			TH1D *Ep = (TH1D*)E->Get("prompt");
			TH1D *Eb = (TH1D*)E->Get("fromb");
