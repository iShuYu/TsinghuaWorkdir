...
int main(int argc, char* argv[])
{
	...
	/*TGraphErrors *ge = new TGraphErrors(38,CX,CM,zeros,CS);
        ...
        ge -> GetXaxis()->SetTitle("#font[12]{N}_{fwd}^{PV}/#LT#font[12]{N}_{fwd}^{PV}#GT_{NB}");
	...
	gmeb -> SetMarkerSize(1.5);
        ...
        gmeb -> GetXaxis()->SetTitle("#font[12]{N}_{fwd}^{PV}/#LT#font[12]{N}_{fwd}^{PV}#GT_{NB}");
	...
	/*TLegend *Title = new TLegend(0.2,0.85,0.9,0.94);
        ...
        Title->Draw("same");*/
	lhcbName->Draw();
        can->SaveAs(Form("../4_Plot/AllF.pdf"),"recreate");
	can->SaveAs(Form("../4_Plot/AllF.png"),"recreate");
	can->SaveAs(Form("../4_Plot/AllF.C"),"recreate");
	can->SaveAs(Form("../4_Plot/AllF.eps"),"recreate");
        can->Close();
	return 0;
}
