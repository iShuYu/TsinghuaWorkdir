...
int main(int argc, char* argv[])
{
	...
	for(int i=0;i<Nybin;i++)
        {
		trigger[i] = (TH1D*)input.Get(Form("trigger%d",i));
                trigger[i]->SetLineColor(color[i]);
                trigger[i]->SetMarkerColor(color[i]);
                trigger[i]->SetMarkerStyle(marker[i]);
                if(i==0) trigger[i]->Draw();
                else trigger[i]->Draw("same");
        }
	for(int i=0;i<Nybin;i++)
	{
        	leg->AddEntry(Form("trigger%d",i), Form("%1.1f < #font[12]{y} < %1.1f", y[i], y[i+1]));
        }
	leg->Draw();
	c1->SaveAs(argv[3]);
        c1->Close();
