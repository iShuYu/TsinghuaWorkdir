...
// Wed Jul 14 16:17:23 2021 by ROOT version 6.22/06
// from TTree DecayTree/DecayTree
// found on file: SPD1_MagUp.root
//////////////////////////////////////////////////////////

#ifndef After_rec_sel_h
#define After_rec_sel_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   After_rec_sel(TTree *tree=0);
   virtual ~After_rec_sel();
...
#ifdef After_rec_sel_cxx
After_rec_sel::After_rec_sel(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("SPD1_MagUp.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("SPD1_MagUp.root");
      }
...
After_rec_sel::~After_rec_sel()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t After_rec_sel::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t After_rec_sel::LoadTree(Long64_t entry)
{
...
void After_rec_sel::Init(TTree *tree)
{
...
void After_rec_sel::Show(Long64_t entry)
{
...
Int_t After_rec_sel::Cut(Long64_t entry)
{
