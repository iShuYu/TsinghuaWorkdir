...
int main(int argc, char *argv[])
{
	...
	C -> Add("../../../Yield/rawdata/1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_i.root");
	C -> Add("../../../Yield/rawdata/1_Rootfile/MC/MagDown/Psi2S/MCPsi2S16Down_i.root");
	C -> Add("../../../Yield/rawdata/1_Rootfile/MC/MagUp/Psi2S/MCPsi2S16Up_l.root");
        C -> Add("../../../Yield/rawdata/1_Rootfile/MC/MagDown/Psi2S/MCPsi2S16Down_l.root");
	MCPsi2S t(C);
	//output
	...
	for(long jentry = 0; jentry <= Num_Entries; jentry++)
	{
		...
		if(!(
			t.nPVs == 1 &&
			t.PVNTRACKS[0] >=n[num_n-1] &&
			t.PVNTRACKS[0] < n[num_n]   &&
			psi_Y  >= y[num_y-1]      &&
                        psi_Y < y[num_y]          &&
