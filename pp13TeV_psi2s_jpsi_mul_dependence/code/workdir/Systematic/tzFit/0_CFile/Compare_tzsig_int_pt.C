...
int main(int argc, char *argv[])
{
	...
	TH1D *Tb = (TH1D*)Tb1->ProjectionX();
	TH1D *Tp = (TH1D*)Tp1->ProjectionX();
        ...
        for(int pt=1;pt<=npt;pt++)
	{
		...
		Tb -> SetBinContent(pt,(Sum_SigPb/Sum_SigJb-Sum_rSigPb/Sum_rSigJb)/(Sum_rSigPb/Sum_rSigJb));
		Tp -> SetBinContent(pt,(Sum_SigPp/Sum_SigJp-Sum_rSigPp/Sum_rSigJp)/(Sum_rSigPp/Sum_rSigJp));
	}
	Tp->Write("Errp");
	Tb->Write("Errb");
	output->Close();
