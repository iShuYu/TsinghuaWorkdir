...
int main(int argc, char *argv[])
{
	...
	for(int y=0;y<ny;y++)
		...
		for(int i=0;i<200;i++)
		{
			mean[y]=mean[y]+Content[y][i];
		}
		mean[y]=mean[y]/200;
		std[y]=0;
		for(int i=0;i<200;i++)
                {
			std[y]=std[y]+(Content[y][i]-mean[y])*(Content[y][i]-mean[y]);
		}
	...
	TH1D *GausErrTable = (TH1D*)GausErrTable2D->ProjectionY();
	//Fit and Fill TH1D
	TCanvas *can[ny];
        for(int y=0;y<ny;y++)
        {
