...
int main(int argc, char *argv[])
{
	...
	for(int i=0;i<200;i++)
	{
		...
		for(int pt=0;pt<npt;pt++)
		{
			...
			Content[pt][i]=Sum_SigP/Sum_SigJ;
		}
		inputJ -> Close();
		inputP -> Close();
		JAcc -> Close();
	...
	for(int pt=0;pt<npt;pt++)
		...
		for(int i=0;i<200;i++)
		{
			mean[pt]=mean[pt]+Content[pt][i];
		}
		mean[pt]=mean[pt]/200;
		std[pt]=0;
		for(int i=0;i<200;i++)
                {
			std[pt]=std[pt]+(Content[pt][i]-mean[pt])*(Content[pt][i]-mean[pt]);
		}
		std[pt]=sqrt(std[pt]/199);
		th[pt] = new TH1D(Form("thpt%d",pt),Form("thpt%d",pt),num_bin,mean[pt]-3*std[pt],mean[pt]+3*std[pt]);
	}
	//output
	...
	TCanvas *can[npt];
        for(int pt=0;pt<npt;pt++)
        {
                for(int i=0;i<200;i++)
                {
			th[pt]->Fill(Content[pt][i]);
		}
		can[pt] = new TCanvas(Form("can%d",pt),Form("can%d",pt),800,600);
		th[pt]->SetLineColor(kRed);
		th[pt]->Draw();
		th[pt]->Fit("gaus");
		TF1 *g = (TF1*)th[pt]->GetListOfFunctions()->FindObject("gaus");
		GausErrTable->SetBinContent(pt+1,g->GetParameter(2)/g->GetParameter(1));
		if(Type==1) can[pt]->SaveAs(Form("../4_Plot/prompt_int_pt%d.pdf",pt+1));
		if(Type==2) can[pt]->SaveAs(Form("../4_Plot/fromb_int_pt%d.pdf",pt+1));
		if(Type==3) can[pt]->SaveAs(Form("../4_Plot/total_int_pt%d.pdf",pt+1));
		can[pt]->Close();
	}
	GausErrTable->Write();
	Out->Close();
