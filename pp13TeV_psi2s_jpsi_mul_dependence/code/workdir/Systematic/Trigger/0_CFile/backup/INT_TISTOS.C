...
int main(int argc, char *argv[])
{
	double Sum_SigJ = 0;
        ...
        double Sum_MSigJ = 0;
        double Sum_MSigP = 0;
	ofstream out("../3_Result/TriggerErr/INTErr.txt");
        for(int Type=1;Type<=2;Type++){
	for(int N=1;N<=5;N++){
        TFile *input;
	input = TFile::Open("../3_Result/TriggerErr/TriggerErr.root");
	...
	out << (Sum_SigP/Sum_SigJ-Sum_MSigP/Sum_MSigJ)/(Sum_MSigP/Sum_MSigJ) << endl;
	}
	out.close();
	return 0;
}
