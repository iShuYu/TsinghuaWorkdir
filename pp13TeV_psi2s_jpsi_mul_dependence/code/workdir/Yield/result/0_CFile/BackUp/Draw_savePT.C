...
int main(int argc, char* argv[])
{
	...
	TGraphErrors *TGEp[5];
	TGraphErrors *TGEb[5];
	TLegend *legp = new TLegend(0.18,0.19,0.55,0.44);
	TLegend *legb = new TLegend(0.18,0.19,0.55,0.44);
	...
	for(int th=0;th<5;th++)
	{
		...
		{	
			TF1 *unit = new TF1("unit","1+x-x",0,4.5);
                	...
                	unit -> GetXaxis()->SetTitle("Normalized #font[12]{n}_{tracks}");
		...
		TGAEp[th] = new TGraphAsymmErrors(N,xp,yp,0,0,yp_lo,yp_hi);
                ...
                TGAEp[th] -> GetXaxis()->SetTitle("Normalized #font[12]{n}_{tracks}");
		...
		TGEb[th] -> Draw("E2PZsame");
                legp -> AddEntry(TGEp[th],Form("%.1f<#font[12]{p}_{T}<%.1fGeV/c",pt_s[th]/1000.001,pt_s[th+1]/1000.001));
		legb -> AddEntry(TGEb[th],Form("%.1f<#font[12]{p}_{T}<%.1fGeV/c",pt_s[th]/1000.001,pt_s[th+1]/1000.001));
	}
	canp -> cd();
	legp -> Draw("same");
	lhcbName ->Draw();
	...
	lhcbName->Draw();
	legb -> Draw("same");
	TLegend *Titlep = new TLegend(0.18,0.44,0.48,0.49);
	Titlep->SetTextFont(132);
        Titlep -> SetTextSize(0.05);
        Titlep -> SetHeader("prompt");
	TLegend *Titleb = new TLegend(0.18,0.44,0.48,0.49);
        Titleb->SetTextFont(132);
	Titleb -> SetTextSize(0.05);
        Titleb -> SetHeader("from #font[12]{b}");
        canp -> cd();
	Titlep -> Draw("same");
	canb -> cd();
	Titleb -> Draw("same");
