...
int main(int argc, char* argv[])
{
	...
	for(int th=0;th<3;th++)
	{
		...
		TGEb[th] -> Draw("E2PZsame");
                legp -> AddEntry(TGEp[th],Form("%.1f<y<%.1f",y_s[th],y_s[th+1]));
		legb -> AddEntry(TGEb[th],Form("%.1f<y<%.1f",y_s[th],y_s[th+1]));
	}
	canp -> cd();
	legp -> Draw("same");
	canb -> cd();
	legb -> Draw("same");
	TLegend *Titlep = new TLegend(0.18,0.44,0.55,0.49);
        Titlep -> SetTextSize(0.05);
        Titlep->SetTextFont(132);
	Titlep -> SetHeader("prompt");
	TLegend *Titleb = new TLegend(0.18,0.44,0.55,0.49);
	...
	lhcbName->Draw();
	Titleb -> Draw("same");
	canp -> SaveAs("../4_Plot/promptRatioY.pdf");
	canp -> Close();
	canb -> SaveAs("../4_Plot/frombRatioY.pdf");
	canb -> Close();
	return 0;
}
