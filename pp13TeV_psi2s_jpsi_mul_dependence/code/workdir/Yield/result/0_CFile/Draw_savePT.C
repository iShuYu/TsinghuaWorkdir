...
int main(int argc, char* argv[])
{
	...
	for(int th=0;th<5;th++)
	{
		...
		{	
			TF1 *unit = new TF1("unit","1+x-x",0,4.4);
                	...
                	unit -> GetXaxis()->SetTitle("#font[12]{N}_{tracks}^{PV}/#LT#font[12]{N}_{tracks}^{PV}#GT_{NB}");
		...
		//gmep[th] -> GetXaxis()->SetTitle("#font[12]{N}_{tracks}^{PV}/#LT#font[12]{N}_{tracks}^{PV}#GT_{NB}");
	...
	canp -> SaveAs("../4_Plot/promptRatioPT.png");
	canp -> SaveAs("../4_Plot/promptRatioPT.C");
	canp -> SaveAs("../4_Plot/promptRatioPT.eps");

	canp -> Close();
	canb -> SaveAs("../4_Plot/frombRatioPT.pdf");
	canb -> SaveAs("../4_Plot/frombRatioPT.png");
	canb -> SaveAs("../4_Plot/frombRatioPT.eps");
	canb -> SaveAs("../4_Plot/frombRatioPT.C");
