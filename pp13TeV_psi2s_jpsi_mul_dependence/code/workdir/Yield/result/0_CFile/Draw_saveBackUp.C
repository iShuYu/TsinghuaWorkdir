...
int main(int argc, char* argv[])
{
	...
	TGraphErrors *ge = new TGraphErrors(38,CX,CM,zeros,CS);
        ...
        ge -> GetXaxis()->SetTitle("#font[12]{N}_{tracks}^{PV}/#LT#font[12]{N}_{tracks}^{PV}#GT_{NB}");
	...
	gmep -> SetMarkerColor(kRed);
   	...
   	gmep -> GetAttFill(1)->SetFillStyle(0);
	gmep -> GetYaxis() -> SetRangeUser(0.6,1.4);
        gmep -> GetXaxis() -> SetLimits(0,7.9279753);
        gmep -> GetXaxis()->SetTitle("#font[12]{N}_{tracks}^{PV}/#LT#font[12]{N}_{tracks}^{PV}#GT_{NB}");
        gmep -> GetYaxis()->SetTitle("Normalised #sigma_{#psi(2S)}/#sigma_{J/#psi}");
	gmep -> Draw("PS same; Z ; 5 s=0.5 ; same");
 	//fromb
	...
	gmeb -> SetMarkerSize(1.5);
        ...
        gmeb -> GetAttFill(1)->SetFillStyle(0);
        gmeb -> GetYaxis() -> SetRangeUser(0.6,1.4);
        gmeb -> GetXaxis() -> SetLimits(0,7.9279753);
        gmeb -> GetXaxis()->SetTitle("#font[12]{N}_{tracks}^{PV}/#LT#font[12]{N}_{tracks}^{PV}#GT_{NB}");
        gmeb -> GetYaxis()->SetTitle("Normalised #sigma_{#psi(2S)}/#sigma_{J/#psi}");
        gmeb -> Draw("PS same; Z ; 5 s=0.5 ; same");
	...
	can->SaveAs(Form("../4_Plot/All.png"),"recreate");
	can->SaveAs(Form("../4_Plot/All.C"),"recreate");
	can->SaveAs(Form("../4_Plot/All.eps"),"recreate");
        can->Close();
	return 0;
}
