void  drawMCDATAb()
{
	TFile *out1 = TFile::Open("newVarMCb.root");
	...
	DecayTree->Draw("PVNTRACKS:PVZ>>h1","fromb_sw");
	TProfile *Pro = h1->ProfileX();
	Pro->SetStats(0);
	Pro->SetLineColor(kBlue);
	Pro->SetTitle("2016 pp 13TeV Data (non-prompt_sw)");
	...
	leg->Draw("same");
	can -> SaveAs("DataMCb.pdf");
	can -> Close();
	TFile *outF = new TFile("./weightb.root","recreate");
	Pro2->Write();
	outF->Close();
}
