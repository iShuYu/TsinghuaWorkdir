...
int main(int argc, char* argv[])
{
	...
	cout << "mean" << endl << M << endl << E << endl;
        RooRealVar mass_mean("mass_mean", "mass mean", fmass_mean, L, H);
	M = fmass_sigma1;
	E = fmass_sigma1Err;
	L = M - C*E;
        H = M + C*E;
	...
	cout << "sigma1" << endl << M << endl << E << endl;
        RooRealVar mass_sigma1("mass_sigma1", "mass sigma1", fmass_sigma1, L, H);
	M = fmass_sigma2;
        E = fmass_sigma2Err;
	L = M - C*E;
        H = M + C*E;
	...
	cout << "sigma2" << endl << M << endl << E << endl;
        RooRealVar mass_sigma2("mass_sigma2", "mass sigma2", fmass_sigma2, L, H);
	M = fmass_ratio;
	E = fmass_ratioErr;
	L = M - C*E/2;
        H = M + C*E/2;
	...
	cout << "ratio" << endl << M << endl << E << endl;
        RooRealVar mass_ratio("mass_ratio", "mass ratio", fmass_ratio, L, H);
        M = fmass_p0;
	E = fmass_p0Err;
	L = M - 2*C*E;
        H = M + 2*C*E;
	cout << "p0" << endl << M << endl << E << endl;
	RooRealVar mass_p0("mass_p0", "mass p0", fmass_p0, L, H);
	...
	//result from background 
        ...
        RooRealVar sigma1("sigma1", "sigma1", 1.4236,0.01,5.);
        RooRealVar sigma2("sigma2", "sigma2", 0.6816,0.01,5. );
        RooRealVar beta("beta", "beta", 0.5, 0.0, 1.);
        ...
        RooRealVar Nprompt("Nprompt", "Nprompt", 0.6*N, 0, N);
        RooRealVar Nfromb("Nfromb", "Nfromb", 0.15*N, 0, N);
        RooRealVar Ntail("Ntail", "Ntail", 0.01*N, 0., 0.2*N);
	...
	RooFitResult *R = tzPDF.fitTo(data_set, Minos(false), Strategy(2), NumCPU(20), Save(), ConditionalObservables(tzError));
        int iter = 0;
	while(R->covQual()!=3||R->edm()>1)
	{
		...
		R = tzPDF.fitTo(data_set, Minos(false), Strategy(2), NumCPU(20), Save(), ConditionalObservables(tzError));
		iter = iter + 1;
		cout << "iter " << iter << endl;
		if(iter>20) break;
	}
