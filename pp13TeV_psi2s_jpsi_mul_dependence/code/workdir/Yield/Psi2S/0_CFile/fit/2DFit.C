...
int main(int argc, char* argv[])
{
        ...
        double fmass_mean, fmass_sigma1, fmass_sigma2, fmass_ratio, fmass_p0, fmass_alpha1, fmass_alpha2, fmass_meanErr, fmass_sigma1Err, fmass_p0Err, mass_Nbkg, mass_Nsig, mass_NbkgErr, mass_NsigErr;
        ifstream fmass(argv[3]);
        fmass >> fmass_mean >> fmass_sigma1 >> fmass_sigma2 >> fmass_ratio >> fmass_p0 >> fmass_alpha1 >> fmass_alpha2 >> fmass_meanErr >> fmass_sigma1Err >> fmass_p0Err >> mass_Nsig >> mass_NsigErr >> mass_Nbkg >> mass_NbkgErr;
        fmass.close();
	double a0, b0, c0, l0;
	a0 = -0.0001082;
        b0 = 0.008496;
	...
	double L, H;
	int C = 10;
	L = fmass_mean - C*fmass_meanErr;
	H = fmass_mean + C*fmass_meanErr;
	cout << L << endl << H << endl;
        RooRealVar mass_mean("mass_mean", "mass mean", fmass_mean);
	L = fmass_sigma1 - 2*C*fmass_sigma1Err;
	H = fmass_sigma1 + 2*C*fmass_sigma1Err;
	if(L<0) L=0;
	cout << L << endl << H << endl;
        RooRealVar mass_sigma1("mass_sigma1", "mass sigma1", fmass_sigma1);
        //RooRealVar mass_sigma2("mass_sigma2", "mass sigma2", fmass_sigma2);
	RooFormulaVar mass_sigma2("mass_sigma2","wider sigma","l+mass_sigma1",RooArgSet(l,mass_sigma1));
	RooRealVar mass_ratio("mass_ratio", "mass ratio", 0.96);
        L = fmass_p0 - 2*C*fmass_p0Err;
	H = fmass_p0 + 2*C*fmass_p0Err;
	cout << L << endl << H << endl;
	...
	//result from background 
        ...
        RooRealVar sigma1("sigma1", "sigma1", 1.4236,0.01,8.);
        RooRealVar sigma2("sigma2", "sigma2",0.6816,0.01,8. );
        RooRealVar beta("beta", "beta", 0.6, 0.0, 1.);
        RooRealVar tau("tau", "tau" ,1.5,0.02,8);
        ...
        RooRealVar Nprompt("Nprompt", "Nprompt", 0.2*N, 0, N);
        RooRealVar Nfromb("Nfromb", "Nfromb", 0.1*N, 0, N);
        RooRealVar Ntail("Ntail", "Ntail", 0.001*N, 0., 0.2*N);
        //RooRealVar Nbkg("Nbkg", "Nbkg", mass_Nbkg, mass_Nbkg-10*mass_NbkgErr, mass_Nbkg+10*mass_NbkgErr);
	RooRealVar Nbkg("Nbkg", "Nbkg", 0.7*N, 0, N);
