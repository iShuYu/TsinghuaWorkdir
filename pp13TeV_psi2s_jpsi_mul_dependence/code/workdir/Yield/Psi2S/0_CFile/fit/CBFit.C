...
int main(int argc, char *argv[])
{
	...
	l0 = 25.7;
	//fit
	RooRealVar psi_mass("psi_M", "psi_mass", 3686 - 120, 3686 + 120);
	RooDataSet data_set("data_set", "data set", input_tree, RooArgSet(psi_mass));
	double N = data_set.numEntries();
	cout<<"total entries = "<< N <<endl;
	RooRealVar mean("mean","mean mass of Psi2S",3686,3670,3695);
	RooRealVar sigma1("sigma1","sigma of CB1",30.,0.,50.);
	RooRealVar l("l","l",l0);
	...
	RooRealVar ratio("ratio","ratio of CB1 over CB1+CB2",0.96);
	RooRealVar p0("p0","p0(for background)",-0.0001,-0.1,0.01);
	...
	RooRealVar Nsig("Nsig", "number of signal", N*0.5, 0, N);
	RooRealVar Nbkg("Nbkg", "number of background", N*0.5, 0 , N);
	...
	leg->AddEntry(frame1->findObject("total"), "total fit function");
	leg->AddEntry(frame1->findObject("signal"), "#psi(2S) signal");
