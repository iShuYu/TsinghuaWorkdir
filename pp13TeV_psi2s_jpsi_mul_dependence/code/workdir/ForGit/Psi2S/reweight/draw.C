...
int main(int argc, char* argv[])
{
	...
	TH1D *Dp_N = new TH1D("Dp_N","Dp_N",50,0,200);
	TH1D *Db_N = new TH1D("Db_N","Db_N",50,0,200);;
	...
	D_N -> SetStats(0);
	Sp_N -> SetLineColor(4);
	Sp_N -> SetTitle("prompt J/#psi");
	Sp_N -> GetYaxis() -> SetRangeUser(0,0.07);
	Sp_N -> GetXaxis() -> SetTitle("PVNTRACKS");
	Sp_N -> Draw("HIST");
	D_N -> SetLineColor(2);
	D_N -> Draw("HIST same");
	Dp_N -> SetStats(0);
	Dp_N -> SetLineColor(3);
        ...
        Dp_N -> Draw("same");
	TLegend *legp = new TLegend(0.65,0.65,0.90,0.90);
	...
	TCanvas *canb = new TCanvas("canb","canb",800,600);
        ...
        D_N -> SetStats(0);
        Sb_N -> SetLineColor(4);
        Sb_N -> SetTitle("J/#psi from b");
        Sb_N -> GetYaxis() -> SetRangeUser(0,0.07);
        Sb_N -> GetXaxis() -> SetTitle("PVNTRACKS");
        Sb_N -> Draw("HIST");
        D_N -> SetLineColor(2);
	D_N -> Draw("HIST same");
        Db_N -> SetStats(0);
        ...
        Db_N -> Draw("same");
        TLegend *legb = new TLegend(0.65,0.65,0.90,0.90);
        legb -> AddEntry(D_N,"MC w/o reweight");
        legb -> AddEntry(Db_N,"MC with reweight");
        legb -> AddEntry(Sb_N,"from-b s-weight");
        legb -> Draw("same");
        canb -> SaveAs(argv[3]);
	canb -> Close();
	return 0;
