...
int main(int argc, char* argv[])
{
	...
	TFile reweight(argv[3]);
        TH1D *reweightpN =(TH1D*)reweight.Get("reweightpN");
        //TH1D *reweightY =(TH1D*)reweight.Get("reweightY");
        TH2D *reweightpPTY =(TH2D*)reweight.Get("reweightpPTY");
	//input trackcalib
	...
	for(int i=0; i<NU; i++)
	{
		...
		double nweight, ptyweight, yweight;
                nweight = reweightpN->GetBinContent(reweightpN->FindBin(AU.PVNTRACKS[0]));
                ptyweight = reweightpPTY->GetBinContent(reweightpPTY->FindBin(PT,Y));
	...
	for(int i=0; i<ND; i++)
		...
		double nweight, ptyweight, yweight;
                nweight = reweightpN->GetBinContent(reweightpN->FindBin(AD.PVNTRACKS[0]));
                ptyweight = reweightpPTY->GetBinContent(reweightpPTY->FindBin(PT,Y));
                //yweight = reweightY->GetBinContent(reweightY->FindBin(Y));
