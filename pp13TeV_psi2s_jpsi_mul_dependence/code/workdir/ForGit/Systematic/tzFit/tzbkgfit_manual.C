...
int main(int argc, char* argv[])
{
	...
	RooDataSet data_set("data_set", "data set", data_tree, RooArgSet(tz, tzError, bkgSweight), 0, "bkg_sw");
        ...
        RooRealVar tau2("tau2", "tau2", 0.055, 0.02, 8);
        RooRealVar tau3("tau3", "tau3", 0.25, 0.1, 5);
	...
	RooFitResult *R = tzPDF.fitTo(data_set, Minos(false), Strategy(2), NumCPU(20), Save(true), ConditionalObservables(tzError));
	int iter = 0;
