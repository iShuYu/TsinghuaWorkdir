...
int main(int argc, char *argv[])
{
	...
	RooRealVar sigma3("sigma3","sigma of CB3",15.,0.,40.);
	RooRealVar a("a","a",a0);
	RooRealVar b("b","b",b0);
	RooRealVar c("c","c",c0);
	...
	RooFormulaVar alpha3("alpha3","alpha3","a*sigma3*sigma3+b*sigma3+c",RooArgSet(a,b,c,sigma3));
	//RooRealVar n("n","n",1.0, 0.5,2);
	RooRealVar n("n","n",1.0);
	RooRealVar ratio("ratio","ratio of CB1 over CB1+CB2",0.5,0.,1.);
	RooRealVar ratio1("ratio1","ratio 1 of CB1 over CB1+CB2",0.5,0.,1.);
	...
	RooCBShape CB3("CBMass3", "CB_Function 3", psi_mass, mean, sigma3, alpha3, n);
	RooAddPdf signal("signal", "signal_mass", RooArgList(CB1, CB2, CB3), RooArgList(ratio,ratio1));
	...
	massPDF.fitTo(data_set, Minos(true), Strategy(1), NumCPU(40), Save(false));
	massPDF.getParameters(data_set);
	massPDF.fitTo(data_set, Minos(true), Strategy(2), NumCPU(40), Save(false));
	...
	txtresult << Nsig.getError() <<endl;
	txtresult << Nbkg.getVal() <<endl;
        txtresult << Nbkg.getError() <<endl;
	//txtresult << sigma1.getError() << endl;
	//txtresult << sigma2.getError() << endl;
	//txtresult << ratio.getError() << endl;
	//txtresult << p0.getError() << endl;
