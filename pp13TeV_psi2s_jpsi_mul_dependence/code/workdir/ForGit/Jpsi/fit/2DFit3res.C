...
int main(int argc, char* argv[])
{
        ...
        double L, H, M, E; 
        int C = 15;
	double fmass_mean, fmass_sigma1, fmass_sigma2, fmass_ratio, fmass_p0, fmass_alpha1, fmass_alpha2, fmass_meanErr, fmass_sigma1Err, fmass_sigma2Err, fmass_p0Err, fmass_ratioErr;
        ifstream fmass(argv[3]);
        fmass >> fmass_mean >> fmass_sigma1 >> fmass_sigma2 >> fmass_ratio >> fmass_p0 >> fmass_alpha1 >> fmass_alpha2 >> fmass_meanErr >> fmass_sigma1Err >> fmass_sigma2Err >> fmass_p0Err >> fmass_ratioErr;
        fmass.close();
	M = fmass_mean;
	E = fmass_meanErr;
	L = M - 2*C*E;
        H = M + 2*C*E;
	cout << "mean" << endl << M << endl << E << endl;
	...
	L = M - 2*C*E;
        H = M + 2*C*E;
	if(L<0) L=0;
	cout << "sigma1" << endl << M << endl << E << endl;
	...
	L = M - 2*C*E;
        H = M + 2*C*E;
	if(L<0) L=0;
	cout << "sigma2" << endl << M << endl << E << endl;
	...
	if(1) L=0;
	if(1) H=1;
	cout << "ratio" << endl << M << endl << E << endl;
        RooRealVar mass_ratio("mass_ratio", "mass ratio", fmass_ratio);
        M = fmass_p0;
	E = fmass_p0Err;
	L = M - 2*C*E;
        H = M + 2*C*E;
	cout << "p0" << endl << M << endl << E << endl;
	RooRealVar mass_p0("mass_p0", "mass p0", fmass_p0);
        RooRealVar n("n", "n", 1.0);
        RooArgSet *Para = new RooArgSet(mass_mean,mass_sigma1,mass_sigma2,mass_ratio,mass_p0);
	...
	//result from background 
        ...
        RooRealVar sigma3("sigma3", "sigma3", 0.6816,0.01,10. );
	RooRealVar beta1("beta1", "beta1", 0.6479, 0.0, 1.);
        RooRealVar beta2("beta2", "beta2", 0.6479, 0.0, 1.);
	RooRealVar tau("tau", "tau" ,1.4,0.02,5);
	...
	RooGaussModel res3("res3", "resolution model 3 for tz", tz, bias, sigma3, one, tzError);
        RooAddModel res("res", "resolution model for tz", RooArgList(res1, res2, res3), RooArgList(beta1,beta2));
        ...
        RooRealVar Nprompt("Nprompt", "Nprompt", 0.7*N, 0, N);
        RooRealVar Nfromb("Nfromb", "Nfromb", 0.2*N, 0, N);
        RooRealVar Ntail("Ntail", "Ntail", 0.001*N, 0., 0.2*N);
        RooRealVar Nbkg("Nbkg", "Nbkg", 0.1*N, 0, 0.9*N);
	...
	while(R->covQual()!=3||R->edm()>1)
	{
		...
		Ntail.setVal(((RooAbsReal*)Para.find("Ntail"))->getVal());
		mass_mean.setVal(((RooAbsReal*)Para.find("mass_mean"))->getVal());
		mass_sigma1.setVal(((RooAbsReal*)Para.find("mass_sigma1"))->getVal());
		mass_sigma2.setVal(((RooAbsReal*)Para.find("mass_sigma2"))->getVal());
		mass_ratio.setVal(((RooAbsReal*)Para.find("mass_ratio"))->getVal());
		mass_p0.setVal(((RooAbsReal*)Para.find("mass_p0"))->getVal());
		sigma1.setVal(((RooAbsReal*)Para.find("sigma1"))->getVal());
		sigma2.setVal(((RooAbsReal*)Para.find("sigma2"))->getVal());
		sigma3.setVal(((RooAbsReal*)Para.find("sigma3"))->getVal());
		tau.setVal(((RooAbsReal*)Para.find("tau"))->getVal());
		beta1.setVal(((RooAbsReal*)Para.find("beta1"))->getVal());
		beta2.setVal(((RooAbsReal*)Para.find("beta2"))->getVal());
		bias.setVal(((RooAbsReal*)Para.find("bias"))->getVal());
		R = tzPDF.fitTo(data_set, Minos(true), Strategy(2), NumCPU(20), Save(), ConditionalObservables(tzError));
		iter = iter + 1;
		cout << "iter " << iter << endl;
		if(iter>5) break;
	}
	...
	fout << "sigma3 = " << sigma3.getVal() << endl;
        fout << "sigma3Error = " << sigma3.getError() << endl;
        fout << "beta1 = " << beta1.getVal() << endl;
	fout << "beta1Error = " << beta1.getError() << endl;
	fout << "beta2 = " << beta2.getVal() << endl;
	fout << "beta2Error = " << beta2.getError() << endl;
	...
	txtout << bias.getVal() << endl;
        ...
        txtout << beta1.getVal() << endl;
        txtout << beta1.getError() << endl;
