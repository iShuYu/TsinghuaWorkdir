void PVZvsMul_new()
{
	...
	for(int i=3; i<10; i++)
	{
		TH1F *hist=new TH1F(Form("hist%d",i),"PVNTRACKS-PVZ (J/#psi, nPV=1, rawdata)",30,0,210);
		F_inJ->Draw(Form("PVNTRACKS[0]>>hist%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
	...
	leg1->Draw();
	can1->SaveAs("../3_result/PVZ_n/Jpsi_rawdatanew.pdf");
	can1->Close();
	...
	TCanvas *can2=new TCanvas("can2","can2",800,600);
        ...
        for(int i=3; i<10; i++)
        {
                TH1F *hist=new TH1F(Form("hist%d",i),"PVNTRACKS-PVZ (#psi(2S), nPV=1, rawdata)",30,0,210);
                F_inP->Draw(Form("PVNTRACKS[0]>>hist%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
                hist->Scale(1.0/hist->Integral());
                hist->SetLineColor(color[i]);
                hist->SetStats(0);
                hist->Draw("same");
        ...
        leg2->Draw();
	can2->SaveAs("../3_result/PVZ_n/Psi2S_rawdatanew.pdf");
	can2->Close();
	...
	TCanvas *can3=new TCanvas("can3","can3",800,600);
        ...
        for(int i=3; i<10; i++)
        {
                TH1F *hist=new TH1F(Form("hist%d",i),"PVNTRACKS-PVZ (J/#psi, nPV=1, data after cut)",30,0,210);
                Jcut->Draw(Form("PVNTRACKS[0]>>hist%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
                hist->Scale(1.0/hist->Integral());
                hist->SetLineColor(color[i]);
                hist->SetStats(0);
                hist->Draw("same");
	...
	TCanvas *can4=new TCanvas("can4","can4",800,600);
        ...
        for(int i=3; i<10; i++)
        {
                TH1F *hist=new TH1F(Form("hist%d",i),"PVNTRACKS-PVZ (#psi(2S), nPV=1, data after cut)",30,0,210);
                Pcut->Draw(Form("PVNTRACKS[0]>>hist%d",i),Form("psi_OWNPV_Z>=%d && psi_OWNPV_Z<=%d && nPV==1",d[i],d[i+1]),"same");
                hist->Scale(1.0/hist->Integral());
                hist->SetLineColor(color[i]);
                hist->SetStats(0);
                hist->Draw("same");
